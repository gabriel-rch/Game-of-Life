...
class Decoder:
    ...
    def decode(self):
                ...
                for match in re.finditer(r"(\d+)([bo])|([bo])", body_line):
                    ...
                    if match.group(1):
                        for i in range(int(match.group(1))):
                            cells[index].append(1 if match.group(2) == 'o' else 0)
                    
                    # if there is no number, append a single cell
                    else:
                        cells[index].append(1 if match.group(3) == 'o' else 0)
        ...
        for row in cells:
            while len(row) < self.pattern_w:
                row.append(0)
        
        # pad the pattern with dead rows
        while len(cells) < self.pattern_h:
            cells.append([0] * self.pattern_w)
